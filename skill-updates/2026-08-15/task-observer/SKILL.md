...
**Step 2 — Inventory skill metadata and route full reads**
...
First build a metadata inventory without loading every skill body. Record each
skill's name, description, locator, and ownership/updateability
(user-owned custom, plugin-owned, or built-in platform). This inventory is the
auditable universe for the review.

Then determine the full-read scope:

1. If an active cross-cutting principle genuinely applies to every skill, read
   every updateable custom skill's SKILL.md completely.
2. Otherwise, shortlist skills for each observation using the explicit Skill
   field, the skill descriptions, the observation's Suggested Improvement, and
   its general Principle. Do not rely on the explicit target alone.
3. Include uncertain matches rather than excluding them. Record excluded skills
   (individually or by clearly described category) and the reason their metadata
   is unrelated, so the review remains auditable.
4. Read every shortlisted SKILL.md completely before mapping or editing it.
   Load linked references only when their routing instructions make them
   relevant to the proposed change.

Metadata-first routing is a context-control mechanism, not permission to skip
plausible matches. A full-library content sweep remains required when an active
principle has library-wide scope.

Exclude built-in platform skills from being updated — only update custom skills
created by the user.
...
For each OPEN observation, evaluate whether it is relevant to every shortlisted
skill. Do NOT rely solely on the observation's own "Skill" field —
observations may contain general principles that apply more broadly than the
original context suggests. Consider both the specific "Suggested improvement"
and the general "Principle" fields. Before finalising the mapping, recheck the
metadata-only exclusions against each observation's Principle and promote any
plausible match to the full-read shortlist. Build a mapping of skill →
...
3. **Check the weekly review trigger.** Read the timestamp in
   ...
   file doesn't exist or the date is more than 7 days ago, the
   Weekly Comprehensive Review (described in full under its own section)
   is due. Apply a **proportionality rule** before running it: if the
   stale review is unrelated to the current user request and running it
   first would materially delay the requested work, do NOT run it inline.
   Instead, note briefly to the user that the review is overdue, complete
   the user's task first, and surface a concise recommendation to run the
   review afterwards or in a separate session. Run the review before the
   task only when the session has no pending substantive request or the
   user asks for it. If fewer than 7 days have passed, proceed normally.

   *Why: a background quality process should improve the work at hand
   without preempting an unrelated, higher-priority user request.*
...
| Skill inventory? | Inventory metadata first; fully read plausible targets, or every custom skill for library-wide principles |
